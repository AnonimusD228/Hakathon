...
class Player:
    def __init__(self,pos,size,image=None,speed=5,shoot_delay=1):
        self.rect=pg.Rect((0,0),size)
    ...
    def update(self,enemys):
        ...
        if self.cur_shoot_delay==0:
            if keys[pg.K_SPACE]:
                self.bullets.append(Bullet((self.rect.centerx,self.rect.y),7))
...
class Space_invaders:
    def start(self):
        player_size=(150,150)
        self.player=Player((screen_width//2-player_size[0]//2,screen_height-player_size[1]),player_size,speed=10,image="spaceinvaders.png")
